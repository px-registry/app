...
export const RIG_LAW: RigLawV1 = Object.freeze({
  rules: Object.freeze([
    ...
    // rule 9 — v3（2026-06-11 Hiroto 裁定: 「一行目に提案を言い切る。二行目に
    // 式をわかりやすくした形」。実鍵チェックで項目名 verbatim の式が初見の
    // 他人に通じないと判明。禁止・必須規定（「私」/CTA/「話してみる」記載禁止・
    // basisItemId 必須・invalid drop）は一字も変えず維持。private/avoid の
    // 引用禁止は rule 6 が引き続き担う。px-table 原本は v3 同文へ直行同期済み
    // （px-table PR #37 = 7949d1c・2026-06-12・verbatim 一致確認済）。
    "提案は読み手に宛てて書く。読み手は「あなた」、相手は表示名で呼ぶ。「私」は使わない。複数の相手を一枚に混ぜない。\n各提案は3行：\n1行目：提案の中身を一文で言い切る。何を・どんな形で、まで具体に書く。読み手を誘う言い回し（「〜しませんか」「〜しましょう」「ぜひ〜」等）にせず、名詞または言い切りで終える。\n2行目：「あなたの◯◯ × ［相手名］の◯◯」の形で、接点の根拠を書く。◯◯は項目名の写しではなく、初めて読む人に伝わる平易な言い回しにする。新しい解釈は足さない。\n3行目：相手の公開項目から一つだけ、相手の手がかりを平易に書く。専門語・内輪の符号は使わない。\n各提案に、根拠にした相手の公開項目を一つ示す（basisItemId）。示せない提案は出さない。\n「話してみる」とは書かない。締めの誘導文・依頼文・連絡を促す文も書かない（ボタンが担う）。",
  ]) as readonly string[],
