...
// 「話してみる」。」を裁定確定文に置換。
// rule 9 3行版（2026-06-11 Hiroto 裁定）— 2行→3行。
// rule 9 v3（2026-06-11 Hiroto 裁定）— 1行目=言い切り／2行目=平易な式／
// 3行目=手がかり。禁止・必須規定は不変。**px-table 原本（d0b8c29・2行）は
// 未同期** — 次の px-table 便で v3 へ直接同期する（v2 を経由しない）。
const LAW_VERBATIM = [
