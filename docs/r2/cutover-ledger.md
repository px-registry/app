...
| 9 | チャットポートのホスティング分離（R3 再訪） | `/port/mcp` のパスは**安定契約** — ポートの読者が公衆化したら（MCP ディレクトリ掲載等）ホスティングだけ分離する（同一 Pages 同居は law/D1 と同じデプロイ単位で drift を消すための現行解・GOAL 便 黄1 確定） | 設計役 2026-06-13 |
