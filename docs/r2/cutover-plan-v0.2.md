...
MESH_OWNER_ALLOWLIST=owner_ref_1,owner_ref_2,...    # env（content write 可の owner_ref）
MESH_BOOTSTRAP_ALLOWLIST=handle_1,handle_2,...       # env（register=bootstrap 可の passkey handle）
KV:MESH_MODE=off | allowlist | on           # KV（実行時に切替・緊急停止はここ）
effective = min(KV:MESH_MODE, MESH_MODE_CAP)   # 弱い方が勝つ（cap を超えられない）
```
  順序 off < allowlist < on。`effective=allowlist` のとき **content write 可は `MESH_OWNER_ALLOWLIST` の owner_ref だけ**・
  **register（bootstrap）可は `MESH_BOOTSTRAP_ALLOWLIST` の passkey handle だけ**（register の blast radius も 5 人に閉じる）。
- 初回本番: `MESH_MODE_CAP=allowlist` / `KV:MESH_MODE=off` / `MESH_OWNER_ALLOWLIST=5人 owner_ref` /
  `MESH_BOOTSTRAP_ALLOWLIST=5人 handle`（→ effective=off・誰も書かない・誰も registry 行を作らない）。
...
- 判定の正本ロジック = `lib/meet-mesh/mode.ts`（純粋・client と Functions が同一実装を読む）:
  ...
  （off→不可・on→owner_ref 形なら可・allowlist→list 内のみ・owner_ref 不在→不可）・
  `bootstrapAllowed(mode, handle, handleAllowlist)`（off→不可・on→可・allowlist→bootstrap list 内 handle のみ・handle 不在→不可）。unit = `mode.test.ts`（12）。
...
- **register の chicken-egg 解決（2 段判定・Hiroto STOP 2026-06-18 で blast radius を閉じた）**:
  owner_ref は register で mint されるため、register 時点では owner_ref allowlist で判定できない。
  → **off→403。on→可（global on は別 Go）。allowlist のとき:**
    - **既存 owner_ref が解決できる** → `MESH_OWNER_ALLOWLIST`（owner_ref）で判定（INSERT 無しの read だが 5 人に閉じる・外は 403）。
    - **owner_ref 未 mint の初回 register** → `MESH_BOOTSTRAP_ALLOWLIST`（passkey **handle**）で判定（list 外 handle は 403）。
  - 入力の出所はすべて **server 解決**: handle は passkey session（HMAC 検証済・偽装 session は 401）、owner_ref は handle→D1 lookup。
    **client 申告の owner_ref / account は信用しない**（register body の owner_ref は無視＝mint 値が返る）。
  - これで allowlist mode の register（owner/device/epoch registry 行の作成）も bootstrap allowlist の 5 人に閉じる。
    bootstrap 後、admin が mint された owner_ref を `MESH_OWNER_ALLOWLIST` に足して content write を開ける（2 段）。
- **deny / fallback honesty matrix A**（不許可時の挙動・CC が正直に裁定）:
  ...
  | `register`（off / bootstrap allowlist 外 / 既存 owner allowlist 外） | HTTP 403 `mesh_disabled` | mock に誤魔化さず off 表示 | mesh 不有効 or 5 人外 |
...
- **local 既定**（`.dev.vars`・コミットしない）: `MESH_MODE_CAP=on` ＋ `MESH_OWNER_ALLOWLIST=`（空）＋
  `MESH_BOOTSTRAP_ALLOWLIST=`（空）＋ KV `mesh:mode=on` seed ＝ローカルは mesh 全開（建設現場・on mode は両 allowlist 無視）。
  本番は §B.0 の初回構成（cap=allowlist・KV=off・両 allowlist=5 人）。
