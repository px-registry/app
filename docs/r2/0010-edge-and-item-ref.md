...
- **0011 追補（便3・2026-06-12）**: `closed_from TEXT`（遷移元の事実 — 裁定文言が sent 段階の閉じと
  mutual 後の閉じで別語のため。**理由の列ではない** — invariant 4 維持）／`to_name TEXT`（宛先の公開
  pseudonym・送信時点固定・from_name と同格 — a 側 pair 面が相手の離脱後も名前を失わないため）。
  serve は closedByMe の真偽と closedFrom のみ（出自の語り分けは表示に出さない）。
...
## 7. ⛔ 分岐の裁定結果（2026-06-12 便3 指示で確定）

- **backfill**: テスターの mutual は残す・デモ edge（あや等の seed 由来）は消す — 出自で分別。
  デモ seed の識別キーは実装裁量（r15pair_ とは別系統）。実装は**カットオーバー移行プラン便**
  （本番接触＝freeze 解除の判断を伴うため、揃った時点で一旦止めて Go を取る）。
- **contact_note**: 完成版 E2EE 封筒へ移行する前提（本日の完成版を本テストとする）。同上、
  移行プラン便で。
- 旧 ⛔ 記述は下に保存（**どちらの裁定でも 0010 の DDL は不変** — 再ゲート不要のまま）。

### （旧）⛔ 分岐を残す箇所（裁定前の両対応設計・記録として保存）
