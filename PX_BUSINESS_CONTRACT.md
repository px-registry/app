...
正本化: 2026-06-12・c19 追補（Hiroto 提供の復元版を v0.4 と v0.6 の間へ verbatim 挿入）

# 【復元メタ — 原文の一部ではない】
# 本ファイルは 2026-06-12 に復元されたもの。原ファイル（/mnt/user-data/outputs/px-business-contract-v05-draft.md・2026-06-11起草）は読込不能となったため、起草元の会話ログ（2026-06-11「人と人をつなぐ出会いの提案」）から conversation_search で抽出した二断片（塗り替えの梯子1〜4項で重複・一致を確認）を接合して verbatim 復元した。本文の編集ゼロ。裁可: 2026-06-10〜11（Hiroto）。
# 以下、原文。
# ---

# PX_BUSINESS_CONTRACT v0.5 改訂案（起草・Hiroto 裁可待ち）

> 位置づけ：v0.4（2a6be92）への改訂。裁可後、CC が repo に正本として記録し、MEMORY.md index に追加する。
> 改訂の出自（取り出せる記憶として残す）：2026-06-10 収益試算 → Hiroto 判断「Layer C は巨人が動いており水平市場としては獲れない。収益の主は Worker」→ design lead 整理「meet はサブ製品でなく Worker 内で動くエンジン」→ 本改訂。

## 改訂1：収益の北極星の移動

- 旧（v0.4）：北極星 = Layer C Machine Attestation（機械可読基盤への課金）
- **新：北極星 = Layer B 事業者容量課金**（縦の面で一定量以上使う事業者への月額定額）
- 価格の錨は displaced fee：「いま仲介に払っている額の数分の一の定額で、成立手数料0円の仕組みに」。取引そのものからは永久に抜かない。
- 個人・小規模事業者は永久無料（不変）。

## 改訂2：Layer C の再定義（降格ではなく内蔵）

- Layer C は独立の水平市場として追わない（巨人の領域）。
- **縦の面に内蔵された支持層**として生きる：Worker の完了実績＝owner が持つ携帯可能な証明（PX が付ける点数ではない）。発注側の検証需要・EU CRA 等の規制証跡需要には、面の機能として応える。
- 効果：①の価格と防壁を支える（手数料ゼロ＋証明付き、の組み合わせは take-rate 競合が追えない）。

## 改訂3：meet の再定義

- meet は「収益のサブ製品」ではない。**出会いエンジン（owner-held 記憶 × 執事 × 提案）の実験室であり、Worker に内蔵される防壁の心臓**。
- 手数料0%は競合に模倣可能。模倣不能なのは：記憶が面をまたいで貯まること／執事が掲載にない機会を記憶から見つけること／中立性がコードで検査できること。
- 終局図：利用者が増えた meet は、個人（法人含む）の自由取引の場へ自然に育つ。**直接太らせず、縦の面が人と記憶を運び込む**（経路＝コールドスタート対策、の当初設計を維持）。
- 5人テストは出会いエンジンの校正として継続。成果は Worker に流れ込む。

## 改訂4：塗り替えの梯子（参入順序の基準と並び）

基準4条件：搾取率が高い × マッチングが構造的に単純 × **モノ・カネ・身体を預からず完結** × 規制が軽い（募集情報等提供で収まる）。

1. 飲食ギグ（beachhead・確定）— Timee 約30%、時間×場所×スキル、給与直接払い、法的整理済み
2. 現場系ギグ横展開（小売・物流・イベント）— 同機構の再利用
3. スキル委託（20%超・デジタル納品・完了実績が効き始める）
4. 販売/オークション（10%・エスクロー期待＝預かり圧力が強く、非保管の信頼設計成熟後）
5. 不動産・人材紹介・恋愛系 — Layer E（別法人）。触らない判断を維持

## 改訂5：収益試算の記録（2026-06-10 時点・支配変数つき）

- 1年目 ¥43M〜123M／2年目 ¥350M〜670M／3年目 ¥1.2B〜2.2B（保守〜基本）
- 支配変数は**有償店舗数**ただ一つ（3年目 2,500〜4,000店＝東京飲食の3〜5%仮定）。検証はチェーン1社パイロットから。
- 粗利 >90%（非保管・推論は owner の鍵・Cloudflare 従量のみ）＝損益分岐が低く、無料個人網を急がず育てる戦略が資金的に成立。
- 不採用と明記：PX ホスト型 AI の販売（「PX は AI を実行しない」境界に触れる。導入するなら別途裁定）。

## 不変（再確認）

owner 間取引手数料ゼロ／paid boost なし／隠れ ranking なし／PX は証明・保証の主体にならない／持たない・判定しない・預からない／forbidden wording list（whitelist は 成立手数料0円 完全一致のみ）。

---

# PX_BUSINESS_CONTRACT v0.6 ドラフト（v0.5への差分改訂）

裁可: 2026-06-12・裁可者 Hiroto（v0.6 を裁可、c19 canon-burn 便で repo 正本化。以下本文は裁可版 verbatim）

状態: **ドラフト — Hiroto裁可待ち。裁可後にCCがrepo正本へ記録（v0.5と同運用）**
作成: Claude（設計役）／敵対検算: GPT（2026-06-11・部分反対→survivors反映）／裁定: Hiroto
v0.5からの不変: 北極星=Layer B事業者容量課金／価格錨=displaced fee定額／個人・小規模永久無料／Layer Cは縦に内蔵／meet=防壁の心臓／塗り替え梯子5段／PXホスト型AI不採用。

---

## §1 二層目標（新設）

**生存の指標** = 容量課金の有償拠点数 × 月額。初期の現実レンジは国内で年数億〜数十億円、強い密度で国内百億円級、国際込みで数百億円級。**¥150〜600億は中央値ではなく強気レンジ**（2026-06-11 GPT検算で訂正）。

| ケース | 有償拠点 | 月額ARPU | 国内年売上 | 国際倍率 | 成熟期売上 |
|---|---:|---:|---:|---:|---:|
| 最悪 | 5,000 | ¥8,000 | ¥4.8億 | 1.0x | ¥4.8億 |
| 中央 | 30,000 | ¥12,000 | ¥43.2億 | 1.5x | ¥64.8億 |
| 強気 | 80,000 | ¥15,000 | ¥144億 | 3.0x | ¥432億 |
| 超強気 | 120,000 | ¥15,000 | ¥216億 | 4.0x | ¥864億 |

**旗の指標** = active拠点数 × 置換された仲介feeの**保守推計**。下限を過大に置かない：10万拠点×¥50万＝**年¥500億**。兆円級は派遣・紹介・スポットワークの複数領域に深く食い込んだ後の**上振れ目標**であり、起点の約束ではない。
支配変数は引き続き**有償拠点数**。母集団（飲食・小売・物流の事業所数）は経済センサス細分類で再集計する（v0.6時点では桁の確認のみ）。

## §2 旗の指標の測定方法論（新設）

PXは成立を見抜かないため、消した中抜きを台帳で直接測らない。**監査済みレンジ**でのみ語る。

1. **保守台帳**: active拠点ごとに「PXで代替した可能性のある仲介カテゴリ」を月次記録（個別成立・金額は取らない）。
2. **業種別fee table**: 公開相場から conservative/base/high を設定。スポットワークはTimee公式30%をhigh側、baseはその半分以下。**派遣はマージン全額を使わず、事業者粗利相当の保守値のみ**（マージンには社会保険・教育訓練等が含まれる — 厚労省）。
3. **サンプル監査**: 四半期ごとの導入者調査「PXがなければ何を使ったか」。調査票・除外基準・信頼区間を公開。自己申告のみでの計上は禁止。

**除外基準（最低限）**: ①PXがなくても仲介を使わなかった成立（反実仮想） ②継続関係の複数月二重計上 ③派遣マージン中の法定・福利厚生費。
**過大に語らないこと自体をPXの信頼にする。**

## §3 課税しないの機械化 — 四重拘束（v0.5の「取らないリスト」を拡張）

**監査テストv2**:「二人のownerの**成立数・成立額**が増えても、PXの請求額が増えないか。増えるなら禁止。」

| 拘束 | 内容 |
|---|---|
| 定款 | 目的・制約条項「成立額・成果・紹介料に連動する収益を取らない」（単体では特別決議で破られる — 束の一部として） |
| 公開憲章 | 違反時の公開説明義務 |
| コードgate | fee/rank/score/recommend/success-fee/commission 系の混入を機械検査（rig系） |
| 請求gate | 請求単位を seat / capacity / storage / admin に限定 |

**迂回課税の禁止列挙（v0.5既存リストに追記）**: 提案数の従量課金（無料3提案/Pro30提案 型）／AI処理料／保証料／**優先見回り**／品質スコア課金／認証バッジ商売／「成立管理付き」プラン。容量課金は提案数・成立期待値・優先露出に**連動させない**。

## §4 Blitzドクトリン（新設）

**区は城ではなく道場。** Phase 1の区密度は型（口上・儀式・物語）を鍛える学習装置であり、札束で防衛する陣地ではない。札束で焼かれる区はある — 防衛線は地域密度ではなく、①owner-held memoryの可搬性 ②非課税の検査可能性（コード・請求・憲章） ③複数面を跨ぐ記憶。**卒業しても記憶が残ることが、後課税プラットフォームへの最大の対抗策**（焼いた側が課税を始めた日に、卒業生を収穫する）。

## §5 国際の入り方（改訂）

労働紹介としてではなく、**owner-held memoryと公開射影のプロトコル**として入る。各国で先に確認する三点: employment agency該当性／platform work規制／AI透明性義務。優先順位は「fee疲弊の深さ」ではなく**「非課税・非判定・非保管の説明可能性と誤分類されにくさ」を第一基準**、fee疲弊を第二基準とする。候補: 韓国・台湾・ドイツ（いずれも法務一次調査を先行）。

## §6 最大形の言明（新設・GPT起草を基に）

> **PXは、世界中のowner-held memoryを、中央の保管・採点・課税なしに接続し、人と人の間に眠っている仕事・場所・技術・関係を、本人のAIが見つけられるようにする非課税プロトコルである。**

プロダクト上の最大価値は「**検索しないで、届く**」。収益の正直な上限は世界で年数百億〜1,000億円級（中央値はそれ以下 — §1）。旗の最大値は年数兆円級の中抜き消去（上振れ目標）。PXは売上最大化企業ではなく、社会の取引摩擦を消すインフラ — 巨大な税関ではなく、電気・DNS・Gitの側。
※「PXポート」等の新語は命名ゲート未通過。対象分野リストのうちケア・教育隣接はCategory Pass-Gate（Layer E判定）の対象。

## §7 判定しないの防衛（新設・小）

最も破られやすい柱は「判定しない」である（GPT指摘）。進行役のテスト読みは品質スコア・ランキングへ変質させない。提案品質の改善は法（rule9系）と取り込み翻訳（c14系）で行い、**採点装置を作らない**。

## §8 巨人条項 — 「巨人が始めればPXは消滅するか」への答え（2026-06-12追補）

**結論: 消滅しない。理由は感傷でなく構造、五つ。**

1. **中立の共有地は競合する巨人のどれにもホストできない** — 各巨人のプールは自社ユーザーの閉じた池。全AIの利用者が同じ卓に着く共有地は、どのエンジンが勝つかに利害を持たない者にしか運営できない（TCP/IP・DNSが巨人の製品でない理由と同型）。PXの無関係性は共有地の運営資格。
2. **検証可能な四ないは巨人には誓えない** — 受託者責任が「ランキング・収益化の永久放棄」を禁じる。四重拘束（§3）は兆ドル企業が構造的に真似できない信用装置。
3. **巨人規模は判定を要求する** — 数十億人のプールはスパム・安全責任のため中央ランキングを必須にし、その瞬間それは旧来のアグリゲーター。判定しないは知性が辺にある時だけ成立し、辺に知性を置いた巨人はクライアントである。
4. **MEET_FINAL_SPEC §12は巨人を事前吸収している** — チャット完結モードの世界で、巨人の最適手は「最高のOwner AIになる」こと。参入＝最強クライアント＝配布網。死のシナリオは規格により流通のシナリオへ変換済み。
5. **時間の非対称（§4 Blitzの一般化）** — 巨人が後にengagement重力で濁った日、owner-held可搬記憶が卒業生を収穫する。

**正直な分岐**: 巨人が本当に四ないを最後まで守るなら、それはPXになったということであり、使命（中抜きゼロの基盤を社会に置く）は達成される。会社が競争に負けても使命は勝つ。ただしこの分岐の確率は2.により低い。

**この条項が要求する三つの先手**: ①規格と記憶カード形式の公開を先に（シェリングポイント先取り） ②cross-providerプールの先行播種（共有地の網効果） ③四重拘束の公開（信用装置を対外信認に）。

## §9 残課題（v0.6で確定しないもの）

- 定款記載の法的実効性・記載文言 → 法務確認（E束に追加）
- 経済センサス細分類での母集団再集計
- 韓国の雇用サービス規制一次調査
- 旗の指標の初回推計（テスト終了後、§2方法論で）

## 裁可欄

Hiroto: 【裁可 ／ 修正指示】 → 裁可後、CCがrepo正本へ記録（改訂履歴つき）。

## 改訂履歴

- v0.6（2026-06-12）: 二層目標・測定方法論・四重拘束・Blitzドクトリン・国際・最大形・判定しないの防衛・巨人条項
