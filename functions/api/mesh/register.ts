...
//
// 設計: docs/r2/device-mesh-how-v0.3.md §3.2/§3.4 経路1。Phase A。
// bootstrap のゲート（stage）= ownerToken（端末ローカル秘密の保有証明＝既存 participantRef 同一性）。
//   ※production hardening は passkey assertion（§3.4 経路1・別便）。本 stage 実装は ownerToken で束ねる。
...
export const onRequestPost: PagesFunction<MeshEnv> = async ({ request, env }) => {
  ...
  if (!isOwnerRef(raw.ownerRef)) return json({ ok: false, error: "owner_ref" }, 400);
  ...
  const participantRef = await deriveParticipantRef(raw.ownerToken);

  try {
    // Idempotent bootstrap: an existing owner_ref returns as-is (no second owner row).
    const existing = await env.BOARD
      .prepare("SELECT owner_ref, current_epoch FROM r15_owner WHERE owner_ref = ?1")
      .bind(raw.ownerRef)
      .all<{ owner_ref: string; current_epoch: number }>();
    if ((existing.results ?? []).length > 0) {
      return json({ ok: true, ownerRef: raw.ownerRef, existing: true });
    }
